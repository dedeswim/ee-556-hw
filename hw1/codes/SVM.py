...
def main():
    training, testing = np.load(
        'dataset/training.npz'), np.load('dataset/testing.npz')
    ...
    parameter['Lmax'] = 0
    for i in range(n):
        parameter['Lmax'] = np.maximum(
            parameter['Lmax'], LA.norm(A[i], 2)*LA.norm(A[i], 2))
    ...
    # Increased from 100 to see the difference with LSAGD
    parameter['maxit'] = 500
    if LSAGDR_option:
    ...
    colors = [(0, 0, 1), (0, 0.5, 0), (1, 0, 0), (0, 0.75, 0.75), (0.75, 0, 0.75),
              (0.75, 0.75, 0), (0, 0, 0), (0.5, 1, 0.5), (0.5, 0, 0.5), (0.75, 0.25, 0.25)]
    ...
    for i, key in enumerate(x.keys()):
        if key not in ['SGD', 'SAG', 'SVR']:
            ax1.plot(np.array(range(1, info[key]['iter']+1)), info[key]
                     ['fx'] - fs_star, color=colors[i], lw=2, label=key)
    ax1.legend()
    ...
    for key in x.keys():
        if key in ['GD', 'SGD', 'SAG', 'SVR']:
            if key == 'GD':
                ax2.plot(np.array(range(
                    info[key]['iter'])), info[key]['fx'] - fs_star, lw=2, label=key, marker='o')
            else:
                ax2.plot(np.array(
                    range(info[key]['iter']))/float(n), info[key]['fx'] - fs_star, lw=2, label=key)
    ax2.set_xlim(0, 5)
